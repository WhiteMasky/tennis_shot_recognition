...
try:
    ...
    import tensorflow.experimental.tensorrt as tf_trt
    TENSORRT_AVAILABLE = True
except ImportError:
    TENSORRT_AVAILABLE = False
    print("TensorRT not available. Using standard TensorFlow inference.")
...
def convert_to_trt_model(model, precision='FP16'):
    """Convert Keras model to TensorRT optimized model"""
    if not TENSORRT_AVAILABLE:
        return model
        
    print(f"Converting model to TensorRT with {precision} precision...")
    conversion_params = tf_trt.ConversionParams(
        precision_mode=precision,
        maximum_cached_engines=16
    )
    
    converter = tf_trt.Converter(
        conversion_params=conversion_params
    )
    
    converter.convert()
    return converter.build(input_fn=lambda: tf.random.normal([1, 30, 26]))
...
if __name__ == "__main__":
    ...
    if TENSORRT_AVAILABLE and not args.no_tensorrt:
        print("Optimizing model with TensorRT...")
        try:
            m1 = convert_to_trt_model(m1)
            print("TensorRT optimization successful!")
        except Exception as e:
            print(f"TensorRT optimization failed: {e}")
            print("Falling back to standard TensorFlow inference.")
    
    # 预热模型
    print("Warming up model...")
    m1(tf.random.normal([1, 30, 26]))
    ...
    while cap.isOpened():
        ...
        # 等待1毫秒，允许显示更新并检查键盘输入
        k = cv2.waitKey(1) & 0xFF  # 使用0xFF以确保正确的按键代码
        if k == 27 or cv2.getWindowProperty('Frame', cv2.WND_PROP_VISIBLE) < 1:  # ESC键或窗口关闭按钮
            break
